...
MyQuadrant::MyQuadrant(Vector3 frontTopLeft, Vector3 backBottomRight, double width, double height, double depth) {
	rootBox = new Box(frontTopLeft, backBottomRight, width, height, depth);
	rootNode = new Node(nullptr, *rootBox, ++id);
};

void MyQuadrant::BuildTree() {
	if (rootNode == nullptr) {
...
void MyQuadrant::BuildTreeRecursive(Node* node) {
	if (node->mDepth > 1) {
		return;
	}
...
void MyQuadrant::ShowVector(Node* node) {
